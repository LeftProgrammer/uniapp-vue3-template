    ...
    <view class="uv-upload__wrap">
      <template v-if="previewImage">
        <view class="uv-upload__wrap__preview" v-for="(item, index) in lists" :key="index">
          <image
            v-if="item.type && item.type.includes('image')"
          ...
          >
            ...
            <text class="uv-upload__wrap__preview__other__text">
              {{ item.isVideo || (item.type && item.type.includes('video')) ? '视频' : '文件' }}
            </text>
          </view>
          <view
...
<script>
import { func, image, video, array, promise } from './libs/function/test.js'
  ...
  methods: {
    ...
    onPreviewImage(item, index) {
      ...
      uni.previewImage({
        ...
        urls: this.lists
          .filter((item) => this.accept === 'image' || image(item.url || item.thumb))
...
$uv-upload-disabled-opacity: 0.5 !default;
.uv-upload {
  @include flex(column);
  flex: 1;
  &__wrap {
