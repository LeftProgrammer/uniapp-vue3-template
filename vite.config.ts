...
export default ({ command, mode }) => {
  ...
  return defineConfig({
    ...
    plugins: [
      viteMockServe({
        mockPath: 'mock', // 指定存放mock数据的目录
        enable: !!(mode === 'development' && env.VITE_APP_MOCK === 'true'), // 是否启用本地mock
        watchFiles: true, // 监听文件变化
      }),
      UniPages({
        mergePages: true, // 确保插件不会覆盖手动配置的页面
        exclude: ['**/components/**/**.*'],
    ...
    define: {
      ...
      __VITE_APP_MOCK__: JSON.stringify(VITE_APP_MOCK),
    },
    css: {
      postcss: {
        plugins: [
