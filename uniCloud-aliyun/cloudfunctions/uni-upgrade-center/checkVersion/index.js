...
module.exports = async (event, context) => {
  ...
  let { appid, appVersion, wgtVersion, is_uniapp_x = false } = event
    ...
    if (record && record.data && record.data.length > 0) {
      ...
      const stablePublishDb = (() => {
        ...
        if (hasAppPackage && hasWgtPackage) {
          if (compare(wgtVersionInDb.version, appVersionInDb.version) >= 0) return wgtVersionInDb
          else {
